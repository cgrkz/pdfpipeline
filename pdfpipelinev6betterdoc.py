...
class ThinkingModeHandler:
    ...
    def extract_thinking(self, text: str) -> Tuple[str, str]:
        """Extract thinking content and main content from model response"""
...
class DiagnosticsLogger:
    ...
    def log(self, filename: str, content: Any):
        """Log content to a file in the session directory"""
    ...
    def add_metric(self, category: str, key: str, value: float):
        """Add a performance metric to the specified category"""
...
class OptimizedMoondreamHandler:
    ...
    def _compile_model_if_needed(self, num_images: int):
        """Compile model for faster inference when processing many images"""
...
class DocumentManager:
    ...
    def get_all_documents(self) -> List[str]:
        """Get list of all processed document names"""
        return list(self.documents.keys())

    def get_document_index(self, pdf_name: str) -> Optional[VectorStoreIndex]:
        """Get the vector index for a specific document"""
...
class EnhancedPDFProcessor:
    ...
    def create_llamaindex_documents(self, pdf_name: str, raw_text_by_page: List[Dict],
        ...
        # Strategy 3: Enhanced image caption processing with rich context
        ...
        print(f"✅ Created {len(documents)} LlamaIndex documents for {pdf_name} with enhanced context preservation")
        return documents
...
class PDFComparisonEngine:
    ...
    def set_llm(self, llm):
        """Set the LLM for comparison operations"""
        self.llm = llm
...
class EnhancedQueryEngine:
    """Custom query engine with optimized FAISS L2 score handling and improved retrieval"""
...
class MultiPDFRAGApplication:
    ...
    def debug_chat_response(self, query: str, selected_docs: List[str]) -> str:
        ...
        for doc_name in selected_docs:
            ...
            if index:
                ...
                # Sort nodes properly (ascending for L2 distance)
    ...
    def _ensure_vlm_loaded(self):
        """Ensure VLM is loaded when needed"""
...
---------------------------------------"""
    ...
    def chat_with_documents(self, message: str, chat_history: List[Tuple[str, str]],
        ...
        if sources:
            ...
            for source in sorted(sources, key=lambda x: x['score']):  # Sort by score ascending (best first)
                key = f"{source['document']}_p{source['page']}"
                if key not in unique_sources or source['score'] < unique_sources[key]['score']:  # Lower is better
...
def create_gradio_interface():
    ...
    with gr.Blocks(theme=gr.themes.Soft()) as demo:
        ...
        def handle_llm_selection(llm_key):
            """Handle LLM selection and show/hide thinking mode checkbox"""
...
if __name__ == "__main__":
    ...
    print("🚀 Starting Enhanced Multi-PDF RAG System...")
